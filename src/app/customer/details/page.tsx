...
export default function CustomerDetailsPage() {
  ...
  const handleDetailsSubmit = (values: CustomerDetails) => {
    setCustomerDetails(values);
  };

  const handleGoogleSignIn = async () => {
    if (!auth || !googleProvider || !customerDetails) return;
    setIsSigningIn(true);
    try {
    ...
    <div className="flex min-h-screen items-center justify-center bg-background p-4">
      <Card className="w-full max-w-md shadow-lg">
        <CardHeader className="text-center">
          ...
          <CardDescription>
            {customerDetails ? 'Sign in to save your details.' : 'First, tell us a bit about yourself.'}
          </CardDescription>
        </CardHeader>
        <CardContent>
          {!customerDetails ? (
            ...
            <div className="space-y-4">
              ...
              <Button variant="outline" onClick={() => setCustomerDetails(null)} className="w-full">
                Edit Details
              </Button>
            </div>
          )}
