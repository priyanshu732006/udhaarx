...
import { User, ArrowRight, Loader2, LogIn } from 'lucide-react';
...
export default function CustomerDetailsPage() {
  ...
  const checkUserAndRedirect = useCallback(async (fbUser: FirebaseUser) => {
    ...
    if (userDoc.exists()) {
      // User profile exists, proceed to app
      router.push('/customer/scan');
    } else {
      // New user, need to get details
  ...
  // This state is if user is logged in, has profile, but somehow landed here.
      ...
      <div className="flex min-h-screen items-center justify-center bg-background p-4">
        <Card className="w-full max-w-md shadow-lg">
          <CardHeader className="text-center">
            ...
            <CardDescription>You are already signed in. You can proceed to the app.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <Button onClick={() => router.push('/customer/scan')} className="w-full">
      ...
      <Card className="w-full max-w-md shadow-lg">
        <CardHeader className="text-center">
          ...
          <CardTitle className="font-headline text-3xl mt-4">
            {showDetailsForm ? 'Almost There!' : 'Customer Sign In'}
          </CardTitle>
          <CardDescription>
            {showDetailsForm ? 'Please provide a few more details to continue.' : 'Sign in with Google to manage your Udhaar.'}
          </CardDescription>
        </CardHeader>
        <CardContent>
